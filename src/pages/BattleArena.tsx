    ...
    return (
      <div className="space-y-6">
        <div className="text-center mb-6 sm:mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2 font-orbitron">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {opponents.map(opponent => (
            <div
              key={opponent.id}
              className="bg-slate-900/50 border border-slate-700 rounded-lg sm:rounded-xl p-4 sm:p-6 hover:border-purple-500 transition-all duration-300 cursor-pointer group"
              onClick={() => startBattle(opponent)}
            >
              <div className="text-center mb-4">
                <div className="text-4xl sm:text-6xl mb-2">{opponent.avatar}</div>
                <h3 className="text-lg sm:text-xl font-bold text-white font-orbitron">
                  {opponent.name}
                </h3>
                <p className="text-slate-400 text-sm">Level {opponent.level}</p>
              </div>

              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 text-sm">Difficulty:</span>
                  <span className={`px-2 sm:px-3 py-1 rounded-full text-white text-xs sm:text-sm font-semibold bg-gradient-to-r ${getDifficultyColor(opponent.difficulty)} capitalize`}>
                    {opponent.difficulty}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 sm:px-3 py-1 rounded-full text-white text-xs bg-gradient-to-r ${getFactionColor(opponent.faction)}`}>
              ...
              <button className="w-full mt-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all duration-200 group-hover:scale-105 text-sm sm:text-base">
                Challenge!
              </button>
            </div>
          ))}
  ...
  if (battlePhase === 'battle' && selectedOpponent) {
    ...
    return (
      <div className="max-w-4xl mx-auto">
        {/* Battle Header */}
        <div className="bg-slate-900/50 border border-slate-700 rounded-lg sm:rounded-xl p-4 sm:p-6 mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
            <div className="flex items-center space-x-2 sm:space-x-4">
              <div className="text-lg sm:text-2xl">{character.name}</div>
              <div className="text-cyan-400 font-bold text-sm sm:text-base">{userScore} pts</div>
            </div>
            
            <div className="text-center">
              <div className="text-xl sm:text-2xl font-bold text-white font-orbitron">VS</div>
              <div className="flex items-center space-x-2 text-slate-400">
                <Clock className="w-3 h-3 sm:w-4 sm:h-4" />
                <span className={`font-bold text-sm sm:text-base ${timeLeft <= 10 ? 'text-red-400' : 'text-white'}`}>
                  {timeLeft}s
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              <div className="text-orange-400 font-bold text-sm sm:text-base">{opponentScore} pts</div>
              <div className="text-lg sm:text-2xl">{selectedOpponent.avatar} {selectedOpponent.name}</div>
            </div>
          </div>
        ...
        {/* Question */}
        <div className="bg-slate-900/50 border border-slate-700 rounded-lg sm:rounded-xl p-4 sm:p-8">
          <div className="mb-4 sm:mb-6">
            <div className="text-xs sm:text-sm text-purple-400 mb-2">{question.category}</div>
            <h3 className="text-lg sm:text-2xl font-bold text-white mb-4 font-orbitron">
              {question.question}
            </h3>
          </div>
            ...
            {question.options.map((option, index) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className="w-6 h-6 sm:w-8 sm:h-8 bg-slate-700 rounded-full flex items-center justify-center text-white font-bold text-sm sm:text-base">
          ...
          {showResult && (
            <div className="mt-4 sm:mt-6 p-3 sm:p-4 rounded-lg bg-slate-800 border border-slate-600">
                ...
                <div className="flex items-center space-x-2">
                  {selectedAnswer === question.correctAnswer ? (
                    <>
                      ...
                      <span className="text-green-400 font-bold text-sm sm:text-base">Correct! +{question.points} points</span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-red-400 font-bold text-sm sm:text-base">Incorrect!</span>
                    </>
                  )}
                </div>
  ...
  if (battlePhase === 'result' && selectedOpponent) {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center">
        <div className={`bg-slate-900/50 border rounded-lg sm:rounded-xl p-6 sm:p-8 ${
          ...
          <h2 className={`text-2xl sm:text-4xl font-bold mb-4 font-orbitron ${
            won ? 'text-green-400' : 'text-red-400'
          }`}>
            {won ? 'VICTORY!' : 'DEFEAT!'}
          </h2>
          
          <div className="text-lg sm:text-xl text-slate-300 mb-4 sm:mb-6">
            {won 
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div className="bg-slate-800 rounded-lg p-3 sm:p-4">
              <div className="text-cyan-400 font-bold text-xl sm:text-2xl">{userScore}</div>
              <div className="text-slate-400 text-sm">Your Score</div>
            </div>
            <div className="bg-slate-800 rounded-lg p-3 sm:p-4">
              <div className="text-orange-400 font-bold text-xl sm:text-2xl">{opponentScore}</div>
              <div className="text-slate-400 text-sm">Opponent Score</div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
            <button
              onClick={resetBattle}
              className="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all duration-200 text-sm sm:text-base"
            >
              Battle Again
            </button>
            <button
              onClick={() => window.location.href = '/leaderboard'}
              className="px-4 sm:px-6 py-2 sm:py-3 bg-slate-700 text-white font-bold rounded-lg hover:bg-slate-600 transition-all duration-200 text-sm sm:text-base"
            >
              View Leaderboard
            </button>
          </div>
